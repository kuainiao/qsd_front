...
function listIntoGraphic(b) {
    $(b).find("li").mouseover(function() {
        $(this).removeClass("listItem ng-scope").addClass("listCurrent").siblings().removeClass("listCurrent").addClass("listItem ng-scope");
    });
}
...
function editUrlFilter(d, c) {
    ...
    switch (d) {
        ...
        case 7:
            ...
            break;
        case 10:
            b = "http://bgm.qiongsandai.com/zhuanti/zhuanti/page_list?project_id=" + c;
            break;
        case 11:
...
$(function() {
    ...
    showHide(".submission", "submissionHover", ".more_submissionWrap");
    showHide(".login_Info", "login_InfoHover", ".more_submissionWrap");
    showHide(".scoreExchangeWrap", "scoreExchangeHover", ".more_submissionWrap");
...
function ajax_love(article_id, channel, obj) {
    ...
    $.ajax({url: "/ajax_love",type: "post",data: "article_id=" + article_id + "&channel=" + channel,dataType: "json",success: function(data) {
        ...
        if ($(obj).find("em").length > 0) {
            ...
            if ($(obj).parent().find("em").length > 0) {
                $(obj).parent().find("em").html(Number($(obj).parent().find("em").html()) + 1);
            } else {
                $(obj).parent().find(".grey").html("<em>1</em>个赞");
            }
        }
        return;
    }});
...
function choujiang() {
    $(".sub_link").each(function(b) {
        $(this).click(function() {
            $(".submit_btn").show().removeAttr("disabled").css("background", "#f04848");
            $("#choujiangComment").removeAttr("disabled");
            var e = $(this);
            var c = e.parent("span").siblings("h3").find("a").html();
            var d = $.trim($("#navBar_login_Info").find("a:first").html());
            if (d == "" || typeof (d) == "undefined") {
                ...
                $("#cj_title").attr("value", c);
                $(".choujiang form textarea").val("#" + c + "#");
                $(".choujiang").fadeIn();
                $("#cover").fadeIn();
                $(".choujiang_result").hide();
            }
            $(".submit_btn").click(function() {
                var f = $(".choujiang form textarea").val();
                if (f.length == 0) {
                    $(".submit_btn").attr("disabled", "disabled").css("background", "#dcdcdc");
                    $(".choujiang form textarea").val("请输入您想参与的抽奖标签");
                    setTimeout(function() {
                        $(".submit_btn").removeAttr("disabled").css("background", "#f04848");
                    }, 2000);
                }
            });
...
function fixedTop(c, b) {
    $(window).scroll(function() {
        var e = $(window).scrollTop();
        var d = $(b).height();
        if (parseInt(e) > parseInt(d)) {
            $(c).addClass("oFixed").css({"position": "fixed","top": 0,"background-color": "F2e76a8","height": "30px"});
            $(c).find(".oHeight").css({"height": "30px","line-height": "30px"});
            $(c).find("nav").children("a").css({"font-size": "14px"});
            $(".moreNav").children("a").css({"font-size": "14px"});
            $(".share_app_icon").hide();
            if ($(c).find(".loginArea").length == 0) {
                $(b).find(".loginArea").appendTo(".navBar").addClass("oFixed");
            }
        } else {
            $(c).removeClass("oFixed").css({"position": "relative","background-color": "F2e76a8","height": "36px"});
            $(c).find(".oHeight").css({"height": "36px","line-height": "36px"});
            $(c).find("nav").children("a").css({"font-size": "16px"});
            $(".moreNav").children("a").css({"font-size": "16px"});
            if ($(b).find(".loginArea").length == 0) {
                $(c).find(".loginArea").appendTo(".logo_search").removeClass("oFixed");
            }
            $(".share_app_icon").fadeIn(200);
        }
    });
}
$(function() {
    $(".wiki span").click(function() {
        $(".wiki").addClass("active").delay(800).fadeOut();
        setTimeout(function() {
            $("#db_cover").fadeOut();
        }, 800);
    });
    fixedTop(".navBarWrap", ".headerWrap");
});
function home_header_stuff() {
    if (!getCookie("home_header_stuff_cookie")) {
        slideStuff();
    }
}
function closeSlideStuff(b) {
    $(b).parent().fadeOut();
    setCookie("home_header_stuff_cookie", 1, (Date.parse(gettodayend()) - Date.parse(gettoday())) / 1000, "/", ".qiongsandai.com");
}
function slideStuff() {
    $(".slide_stuff").css("display", "block");
    $(".slide_stuff_repeat").css("display", "none");
    $(".slide_stuff_close").css("display", "none");
    $(".slide_stuff").animate({height: "0"}, 600, function() {
        $(".slide_stuff_small").css("display", "none");
        $(".slide_stuff_big").css("display", "inline");
    }).animate({height: "450px"}, 1000).animate({height: "450px"}, 5000).animate({height: "0"}, 1000, function() {
        $(".slide_stuff_big").css("display", "none");
        $(".slide_stuff_small").css("display", "inline");
        $(".slide_stuff_repeat").css("display", "block");
        $(".slide_stuff_close").css("display", "block");
    }).animate({height: "90px"}, 600);
}
function home_cover_stuff() {
    if (!getCookie("home_cover_stuff_cookie")) {
        coverStuff();
    } else {
        if (!getCookie("home_cover_small_stuff_cookie")) {
            coverStuffSmall();
        }
    }
}
function coverStuff() {
    $(".cover_stuff_repeat").css("display", "none");
    $(".cover_stuff_close").css("display", "none");
    $(".cover_stuff").css("display", "block");
    $(".cover_stuff_area").css("display", "block");
}
function coverStuffSmall() {
    $(".cover_stuff").css("display", "none");
    $(".cover_stuff_narrow").css("display", "none");
    $(".cover_stuff_big").css("display", "none");
    $(".cover_stuff_area").css("display", "block");
    $(".cover_stuff_small").css("display", "block");
    $(".cover_close_btn").css("display", "block");
    $(".cover_stuff_repeat").css("display", "block");
    $(".cover_stuff_close").css("display", "block");
    $(".cover_stuff_area").css({"width": "140px","height": "100px","top": "100%","left": "0","margin-top": "-100px","margin-left": "0"});
}
function narrowCoverStuff(b) {
    $(".cover_stuff").css("display", "none");
    $(".cover_stuff_narrow").css("display", "none");
    $(b).parent().animate({"width": "140px","height": "100px","top": "100%","left": "0","margin-top": "-100px","margin-left": "0"}, 600, function() {
        $(".cover_stuff_big").css("display", "none");
        $(".cover_stuff_small").css("display", "block");
        $(".cover_close_btn").css("display", "block");
        $(".cover_stuff_repeat").css("display", "block");
    ...
    if (!getCookie("home_cover_stuff_cookie")) {
        setCookie("home_cover_stuff_cookie", 1, (Date.parse(gettodayend()) - Date.parse(gettoday())) / 1000, "/", ".qiongsandai.com");
    }
}
function repeatCoverStuff(b) {
    $(".cover_stuff").css("display", "block");
    $(".cover_close_btn").css("display", "none");
    $(".cover_stuff_repeat").css("display", "none");
    $(".cover_stuff_close").css("display", "none");
    $(".cover_stuff_small").css("display", "none");
    $(".cover_stuff_big").css("display", "block");
    $(b).parent().animate({"width": "640px","height": "460px","top": "50%","left": "50%","margin-top": "-230px","margin-left": "-320px"}, 600, function() {
        $(".cover_stuff_narrow").css("display", "block");
    });
}
function closeCoverStuff(b) {
    $(b).parent().fadeOut();
    if (!getCookie("home_cover_small_stuff_cookie")) {
        setCookie("home_cover_small_stuff_cookie", 1, (Date.parse(gettodayend()) - Date.parse(gettoday())) / 1000, "/", ".qiongsandai.com");
    }
}
